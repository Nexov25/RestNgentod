...
// Apikey
var listkey = ["RonzzOfc","9286c1a775","9267ic6a0f1","927j59de9c","921n567ea6","921h5a4282","925n2c494","928b0323c9","927b0k3hp7o2","925b04ib0j","023l1qhbpk","92b1a0h7ts","92a70b789c","9291a7bk0p1","92a7o8pe9c","92y1a7l0a6","9221a7i9h2","921a7k3n94","92a0kk2bc9","921a7l9pho2","92a2n1kb0j","92b0a75k6f","92u1a7pr8s"];
var listkeyprem = ["RonzzOfc"];
...
router.get('/downloader/ytplay', async(req, res, next) => {
  ...
  if(listkeyprem.includes(apikey)){
    fetch(encodeURI(`https://ronzz-api.herokuapp.com/api/downloader/ytplay?query=${query}&apikey=RonzzOfc`))
...
router.get('/downloader/ytmp3', async(req, res, next) => {
  ...
  if(listkey.includes(apikey)){
    fetch(encodeURI(`https://ronzz-api.herokuapp.com/api/downloader/ytmp3?url=${query}&apikey=RonzzOfc`))
    .then(response => response.json())
        .then(hasil => {
...
router.get('/downloader/ytmp4', async(req, res, next) => {
  ...
  if(listkey.includes(apikey)){
    fetch(encodeURI(`https://ronzz-api.herokuapp.com/api/downloader/ytmp4?url=${query}&apikey=RonzzOfc`))
    .then(response => response.json())
        .then(hasil => {
...
router.get('/search/youtube', async(req, res, next) => {
  ...
  if(listkey.includes(apikey)){
    fetch(encodeURI(`https://ronzz-api.herokuapp.com/api/search/youtube?query=${query}&apikey=RonzzOfc`))
    .then(response => response.json())
        .then(hasil => {
...
if (!name) return res.json({ status : false, creator : `${creator}`, message : "Enter Name"})
if (!mem) return res.json({ status : false, creator : `${creator}`, message : "Enter Members Amount"})
if (!msg) return res.json({ status : false, creator : `${creator}`, message : "Enter Message Welcome"})
if (!picurl) return res.json({ status : false, creator : `${creator}`, message : "Enter Pic Url"})
if (!bgurl) return res.json({ status : false, creator : `${creator}`, message : "Enter Background Url"})
if(!apikey) return res.json(loghandler.notapikey)
if(listkey.includes(apikey)){
...
var requestSettings = {
url: `https://api-yogipw.herokuapp.com/api/welcome3?name=${name}&mem=${mem}&msg=${msg}&picurl=${picurl}&bgurl=${bgurl}`, method: 'GET', encoding: null };
request(requestSettings, function(error, response, body) {
res.set('Content-Type', 'image/png');
res.send(body)
})
...
var requestSettings = {
url: `https://api-yogipw.herokuapp.com/api/goodbye3?name=${name}&mem=${mem}&msg=${msg}&picurl=${picurl}&bgurl=${bgurl}`, method: 'GET', encoding: null };
request(requestSettings, function(error, response, body) {
res.set('Content-Type', 'image/png');
res.send(body)
})
...
var requestSettings = {
url: `https://lexxyapi.herokuapp.com/api/canvas/verifikasi?nama=${name}&member=${mem}&resi=${sn}&ppnya=${pp}&bgnya=${bg}&apikey=lexxy`, method: 'GET', encoding: null };
request(requestSettings, function(error, response, body) {
res.set('Content-Type', 'image/png');
res.send(body)
})
...
var requestSettings = {
url: `https://api.lolhuman.xyz/api/qrcode?apikey=SadTeams&text=${text}`, method: 'GET', encoding: null };
request(requestSettings, function(error, response, body) {
res.set('Content-Type', 'image/png');
res.send(body)
})
...
router.get('/other/readqrcode', async (req, res, next) => {
	var url = req.query.url
	var apikey = req.query.apikey
	if (!url) return res.json({ status : false, creator : `${creator}`, message : "Please Enter Url"})
	if (!apikey) return res.json(loghandler.notapikey)
	if(listkey.includes(apikey)){
	
	let data = await fetchJson(`https://api.lolhuman.xyz/api/read-qr?apikey=SadTeams&img=${url}`)
	if (!data.result) return res.json(loghandler.notfound)
	res.json({
	status: true,
	creator: `${creator}`,
